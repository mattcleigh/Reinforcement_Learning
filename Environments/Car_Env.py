...
class MainEnv:
    ...
    def __init__(self):
        ...
        self.bck_ray_angles  = np.pi/2*np.linspace(-1.0, 1.0, self.n_bck_rays)
        ...
        self.means = np.array([ 3.4,  0.0,
                                ...
                                14.9, 12.0, 10.3, 9.2,
                                8.6,  8.3,  8.2,
                                8.2, 12.3, 33.3, 12.3, 8.2 ])

        self.deviations = np.array([ 1.9,  0.5,
                                     2.8,   3.4,  3.8,  4.5,
                                     5.7,  7.8,  10.3,  12.9, 15.3, 18.4,
    ...
    def state(self):
        ...
        """
        raw = np.concatenate(( [self.fwd_vel], [self.sid_vel], self.fwd_ray_lens, self.bck_ray_lens ))
        normalised = ( raw - self.means ) / self.deviations
        return normalised
    ...
    def shoot_rays(self):
        ...
        self.bck_ray_end = np.array([ Geometry.rotate_2d_vec( -self.heading, a ) for a in self.bck_ray_angles ])

        self.fwd_ray_end =  self.max_ray_lenths * self.fwd_ray_end + self.car_front
        self.bck_ray_end =  self.max_ray_lenths * self.bck_ray_end + self.car_back

        ## Now we find where the rays collide with the track
        self.find_ray_hit(self.car_front, self.fwd_ray_end)
        self.find_ray_hit(self.car_back,  self.bck_ray_end)
    ...
    def step(self, action):
        ...
        info = 0
        return self.state, reward, failed, info
...
class CarGameWindow(pyglet.window.Window):
    ...
    #     if symbol == key.SPACE: self.env.reset()
    ...
    def draw_vision(self):
        ...
        bck_ray_lens = 0.5*self.scale*self.env.bck_ray_lens
        glColor4f(1.0, 0.0, 0.0, 1)
        glLineWidth(4.0)
    ...
    def draw_rays(self):
        ...
        car_back    = self.scale * self.env.car_back
        fwd_ray_end = self.scale * self.env.fwd_ray_end
        bck_ray_end = self.scale * self.env.bck_ray_end
        glColor4f(1.0, 0.0, 0.0, 1)
        glLineWidth(2.0)
        glBegin(gl.GL_LINES)
