...
class C51DuelMLP(nn.Module):
    ...
    def __init__(self, name, chpt_dir,
        ...
        self.V_stream = myNN.mlp_creator( "V", n_in=width, n_out=n_atoms, w=width, act_h=activ, nsy=noisy )
        self.A_stream = myNN.mlp_creator( "A", n_in=width, n_out=n_actions*n_atoms, w=width, act_h=activ, nsy=noisy )
...
class Agent(object):
    ...
    def train(self):
        ...
        with T.no_grad():
            ...
            new_supports = rewards + self.n_gamma * self.supports * (~dones)
